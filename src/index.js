      ...
      const children = element.children
        .map((node) => {
          ...
          if (
            ...
            return [node, builders.comment('!CHECK TRANSLATION!')];
          }
          // ignore any comments including the !CHECK TRANSLATION! comments
...
// this function will get all of the relevant files, read them, get the relevant translation files, trigger the transform, then rewrite the files
export function runCodemod(options) {
  const filePaths = findFiles('app/**/*.hbs', options);
 ...
 * The codemod will not attempt to change and mustache statements so they will need manual attention. The codemod will add a comment with the string !CHECK TRANSLATION! to make it easier to find
 */
